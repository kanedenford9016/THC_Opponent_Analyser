...
async function fetchWithTimeout(url, options, timeoutMs, label) {
  ...
  try {
    console.log("Torn fetch start", label);
    ...
    // Timer cleared by timeout promise resolving; AbortController handles in-flight fetch.
  }
}
...
export async function fetchPlayerStats(apiKey, playerId, baseUrl = DEFAULT_BASE_URL) {
  ...
  let lastError = null;
  for (let attempt = 1; attempt <= PLAYER_MAX_RETRIES; attempt += 1) {
    try {
      console.log("Player fetch attempt", attempt, playerId);
...
export async function fetchFactionMemberIds(apiKey, factionId, baseUrl = DEFAULT_BASE_URL) {
  ...
  const v1Url = `https://api.torn.com/faction/${factionId}?selections=members&key=${encodeURIComponent(apiKey)}`;
  const response = await fetchWithTimeout(
    v1Url,
    {},
    DEFAULT_TIMEOUT_MS,
    `faction_v1:${factionId}`
  );
  ...
  const membersArray = Array.isArray(data?.members) ? data.members : null;
  const membersObject = data?.members && typeof data.members === "object" ? data.members : null;
  const ids = membersArray
    ? membersArray.map((member) => String(member?.id || ""))
    : membersObject
      ? Object.keys(membersObject)
      : [];
