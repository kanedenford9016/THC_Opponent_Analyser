...
export async function createJob({ userId, apiKey, targetType, rawIds }) {
  await ensureTable();
  const id = crypto.randomUUID();
  await sql`
    INSERT INTO discord_jobs (id, user_id, status, target_type, api_key, raw_ids)
    VALUES (${id}, ${userId}, 'queued', ${targetType}, ${apiKey}, ${rawIds})
  `;
  return id;
}
