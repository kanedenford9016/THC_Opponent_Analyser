...
async function ensureTable() {
  await executeWithRetry(
    async () => {
      console.log("[NEON] ensureTable sql start");
      const result = await buildTableSql();
      console.log("[NEON] ensureTable sql done");
      return result;
    },
    "ensureTable",
    3000
  );
  console.log("[NEON] ensureTable complete");
}
...
async function executeWithRetry(fn, label, timeoutMs = 3000) {
  let lastError;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      console.log(`[NEON] ${label} attempt ${attempt} start`);
      const result = await withTimeout(fn, timeoutMs, `${label} attempt ${attempt}`);
      console.log(`[NEON] ${label} attempt ${attempt} ok`);
      return result;
    } catch (error) {
      lastError = error;
      if (attempt >= 3 || !isRetryableNeonError(error)) {
        console.error(`[NEON] ${label} attempt ${attempt} failed, no retry`, error);
        throw error;
      }
      console.warn(`[NEON_RETRY] ${label} attempt ${attempt} failed, retrying`, error);
...
export async function createJob({ userId, apiKey, targetType, memberIds }) {
  ...
  console.log("[NEON] createJob complete", id);
  return id;
}
